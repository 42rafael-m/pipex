...
/*   Updated: 2025/06/03 19:49:11 by rafael-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_parent(char *outfile, int pipefd, char *cmd, char *p, char **envp)
{
	...
	argv = ft_split(cmd, ' ');
	if (!argv)
		return (free(p), perror("malloc"), errno);
	if (execve(p, argv, envp) == -1)
		return (ft_free_d(argv), free(p), perror("execve"), errno);
	return (errno);
}
...
char	*ft_parse_cmd(char *cmd, char **env)
{
	...
	while (env[i])
	{
		...
		{
			cmd_path = ft_strjoin(env[i] + 5, cmd);
			if (!cmd_path)
				return (perror("malloc"), NULL);
			break ;
		}
...
int	main(int argc, char **argv, char **envp)
{
	...
	status = ft_pipe_fork(argv[1], argv[4], p1, p2, argv[2], argv[3], envp);
	return (free(p1), free(p2), status);
}
